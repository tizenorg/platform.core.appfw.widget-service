...
static int _parse_widget_application(xmlNode *node, GList **list)
{
	...
	if (appid == NULL) {
		LOGE("cannot find appid");
		return -1;
	}

	for (tmp = node->children; tmp; tmp = tmp->next) {
		if (_get_tag(tmp) != TAG_WIDGET_CLASS)
			continue;
		if (_parse_widget_class(tmp, appid, list)) {
			free(appid);
			return -1;
		}
	}
...
/* support 2.4 style widget-application */
static int _parse_widget_application_legacy(xmlNode *node, GList **list)
{
	char *val;
	xmlNode *tmp;
	struct widget_class *wc;
...
GList *widget_plugin_parser_parse_manifest(xmlDocPtr doc)
{
	...
	xmlNode *tmp;
	GList *list = NULL;
	int (*_parse_widget)(xmlNode *, GList **) = _parse_widget_application;
	...
	for (tmp = root->children; tmp; tmp = tmp->next) {
		...
		if (_parse_widget(tmp, &list)) {
			LOGE("parse failed");
			return NULL;
		}

		/* switch to parse 2.4 style widget-application */
		if (g_list_length(list) == 0) {
			_parse_widget = _parse_widget_application_legacy;
			if (_parse_widget(tmp, &list)) {
				LOGE("parse failed");
				return NULL;
			}
