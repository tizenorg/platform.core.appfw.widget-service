...
static const char *__get_widget_db(uid_t uid)
{
	...
	if (uid != tzplatform_getuid(TZ_SYS_GLOBALAPP_USER) && uid != ROOT_UID) {
		...
		widget_db = tzplatform_mkpath(TZ_USER_DB, ".widget.db");
...
{
	int rc;
	char select_query[QUERY_MAX_LEN];
	char *widget_id;
	char *content_info;
	char *instance_id;
	struct _widget_instance *instance = NULL;
	...
	snprintf(select_query, sizeof(select_query), "SELECT widget_id, content_info, instance_id FROM widget_instance WHERE viewer_id='%s'", viewer_appid);
...
int __set_width(bundle *content_info, int w)
{
	char wbuf[6];
...
int __set_height(bundle *content_info, int h)
{
	char hbuf[6];
