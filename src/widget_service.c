...
static bool _is_widget_feature_enabled(void)
{
...
static int _convert_support_size(int *width, int *height, int count)
{
	int i;
	int j;
		...
		int **w, int **h)
{
	...
	if (_convert_support_size(width, height, count)) {
		_E("failed to convert size ratio");
		free(width);
		free(height);
		return WIDGET_ERROR_FAULT;
	}
