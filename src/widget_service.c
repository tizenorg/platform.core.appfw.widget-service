...
static void __instance_list_cb(const char *key, const int type, const bundle_keyval_t *kv, void *user_data)
{
	struct instance_cb *cb_data = (struct instance_cb *)user_data;
	cb_data->cnt++;

	_D("instance list: %s %s", cb_data->widget_id, key);
	if (cb_data && cb_data->cb)
		cb_data->cb(cb_data->widget_id, key, cb_data->data);
}
...
EAPI int widget_service_get_widget_instance_list(const char *widget_id, widget_instance_list_cb cb, void *data)
{
	...
	ret = aul_app_data_get_raw(widget_id, &b);
	if (ret == AUL_R_EILLACC)
		return WIDGET_ERROR_PERMISSION_DENIED;

	if (ret == AUL_R_ENOAPP || b == NULL)
		return WIDGET_ERROR_NOT_EXIST;

	if (ret == 0) {
		bundle_foreach(b, __instance_list_cb, &cb_data);
	}
...
EAPI int widget_service_set_lifecycle_event_cb(const char *widget_id, widget_lifecycle_event_cb cb, void *data)
{
	...
	widget_instance_listen_status(widget_id, __listen_status, cb_info);
...
EAPI int widget_service_unset_lifecycle_event_cb(const char *widget_id, void **user_data)
{
	...
	if (found) {
		...
		widget_instance_unlisten_status(widget_id);
